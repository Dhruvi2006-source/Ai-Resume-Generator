...
import { Plus, CloudUpload, FilePenIcon, FilePenLineIcon, TrashIcon, PencilLine, XIcon, Flag, UploadCloudIcon } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="body  gap-6 flex flex-wrap items-center">
        ...
        <div onClick={()=> setUploadResume(true)} className="cursor-pointer bg-slate-200 w-60 h-80 border m-3 border-dashed border-gray-300 rounded-2xl hover:border-gray-800 flex flex-col justify-center items-center gap-3">
        ...
        {allResumes.map((resume, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-all duration-300 hover:visible">
                <TrashIcon className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-500 transition-colors" />

                <PencilLine className="size-7 p-1.5 hover:bg-white/50 rounded text-slate-500 transition-colors" />
              </div>
            </button>
          );
        })}
